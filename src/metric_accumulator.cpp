...
void MetricsAccumulator::AccumulateNextFunctionResults(const std::vector<metric::MetricResult> &metric_results) const {
    std::ranges::for_each(metric_results, [&](const auto &metric_result) {
        const auto position = accumulators.find(metric_result.metric_name);
        if (position == accumulators.end()) {
            throw std::runtime_error("Accumulator for metric '" + metric_result.metric_name + "' is not registered");
        }
        position->second->Accumulate(metric_result);
    });
}
/**
...
void MetricsAccumulator::ResetAccumulators() {
    std::ranges::for_each(accumulators, [](auto &entry) { entry.second->Reset(); });
}
